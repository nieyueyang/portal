	...
	if(0==$(".calculate:checked").size()){
		...
		var cartId = ""
			...
			art.dialog({
				...
				jQuery.ajax({
					type : "POST",
					url : appServer + "/delisting/buyer/delete.htm",
					data : {
						"cartId" : cartId
					},
 ...
 * @return
 */
function deleteOne(cartId,type,companyId){
	art.dialog({
		id : "del",
		lock : true,
	    content: '确定要删除该条信息吗?',
		...
		jQuery.ajax({
			type : "POST",
			url : appServer + "/delisting/buyer/deleteOne.htm",
			data : {
					"id" : cartId,
					"type" : type
			},
								...
								yesFn : function(){
									if("companyId"==type){
										...
										if(2==$("."+companyId).size() ){
											...
											$("input[value="+cartId+"]").parentsUntil("table").remove();
										}
									}
									calculate();
								}
