...
function formatPrice(){
	var listPrice = $("#listPrice").val();
		...
		var listWeight = $("#listWeight").val();
		var totalPrice = parseFloat(accMul(listPrice,listWeight)).toFixed(2);
		$("#listTotalPriceAmt").val(totalPrice);
	}
}

function formatRealPrice(){
	var listPrice = $("#realTotalPriceAmt").val();
	if(!isNaN(listPrice)){
		if(Number(listPrice) <= 0){
			return;
		}
			listPrice = parseFloat(listPrice).toFixed(2);
			$("#realTotalPriceAmt").val(listPrice);	
	}
}

function formatTotalPrice(){
	var listPrice = $("#listTotalPriceAmt").val();
	if(!isNaN(listPrice)){
		if(Number(listPrice) <= 0){
			return;
		}
			listPrice = parseFloat(listPrice).toFixed(2);
			$("#listTotalPriceAmt").val(listPrice);	
			var listWeight = $("#listWeight").val();
			var unitPrice = parseFloat(accDiv(listPrice,listWeight)).toFixed(2);
			$("#listPrice").val(unitPrice);
	}
}
