...
$(function() {
			...
			.dialog(
					{
						...
						buttons : {
							...
							/*		if (!idList.in_array(element)) {
									...
									$("#directUserCompanyId").val(element.split(":")[0]);
									
									if(element.split(":")[1] == null || element.split(":")[1] == ""){
										$("#directUserCompanyName").val("企业ID:"+element.split(":")[0]);
									}else{
										$("#directUserCompanyName").val(element.split(":")[1]);
									}
	...
	$("#buyerSelectDialog").bind(
			...
			function(event, ui) {
				$("#buyerSelectDialog").load(
						appServer + "/agentListing/list_directMember.htm?"+Math.round((Math.random()) * 100000000), null,
						function() {
							check();
						});
			});//end bind
	...
	$("#buyerSelectDialogForm a[id=listBuyer]").live(
			...
			function() {
				...
				$("#buyerSelectDialog").load(
						appServer + "/agentListing/list_directMember.htm?"+Math.round((Math.random()) * 100000000),
