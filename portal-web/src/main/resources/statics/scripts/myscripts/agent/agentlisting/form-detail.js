...
function createTableByVariety(type){
	...
	var priceUnitType = $("input[name=priceUnitType]").val();//从隐藏域中获取 金额单位
	 ...
	 jQuery.get(        
			 appServer + '/agentListing/load_detail_'+id+'_detail_null.htm?' + Math.round((Math.random()) * 100000000),
	    		function(data){
	    ...
	    /*			if(project.buyType){
	    		...
	    		/*     formHtml = formHtml +  "<th width=\"132px\">参考市场行情 :</th>" + 
                "<td width=\"227px\">" +  (project.referencePrice == null ? "" : project.referencePrice )  +" "+ (priceUnitType ==null ? "元/吨": priceUnitType )+ " </td></tr>"*/  
