		 ...
		 var form = $('#form_step2').validateForm({
		        ...
		        submitHandler:function(f) {
		        	...
		        	if(validateForm()){
		        		f.submit();
		        		/*
						 * Boxy.confirm("挂牌开始日期："+$("#listingDatesStr").val()+"<br />挂牌结束日期："
						 * +$("#listingDateeStr").val() + "<br />交收截止日期："+$("#settlementDate").val()+"<br />确定保存吗？",
						 * function(){ f.submit(); } )
						 */
		        	}
		        }
	 ...
	 jQuery.get(       
			 appServer + '/listing/init_form.htm',
	    		...
	    		function(data){
	    			...
	    			var varietyType = $('input[name=whInventoryVarietyType]').val();
	    			if('2' == varietyType){
	    				if(WHIItems == ''){
		    				 Hundsun.PopUtil.alert({
								msg:"未加载到库存明细，不能挂牌！",
	    			...
	    			/*if($("#listingType").val() =="W" || $("#listingType").val() =="F"){  // 库存挂牌(非库存现有量挂牌),只显示“规格”属性，不显示长、宽、厚等
	    				formHtml += "<tr><th width=\"14%\">规格:</th><td ><input type='text'  id='specification' name='specification' value='";
                   ...
                   +"</td><td id = \"warehouseCmpTd\" colspan=\"2\"><select id = \"warehouseCmp\" name = \"warehouseCmp\" onchange=\"setWHCvalue();\"   style=\"width:150px;\"  class=\" required  \">" +
	    			...
	    			if('2' == varietyType){
		    			...
		    			if(null != WHIItems && "" != WHIItems){
		    				jQuery(WHIItems).each(function(n){
		    					formHtml += '<tr style="border:1px solid #d1d7e0" id="tr_' + this.id + '" >' + 
		    					...
		    					'</td>' + 
		    					'<td class="tdB" style="border:1px solid #d1d7e0;">' + 
		    					'<span class="button-9"><a href="javascript:void(0)" onclick="deleteRows(\'tr_' + this.id + '\', ' + this.weight + ');return false;" >删除明细</a></span>' + 
		    					'</td>' + 
		    					'</tr>';
		    				})
		    			}
	    			...
	    			// 填充属性对照表
	    			if('2' == varietyType){
	    				var attrButeComp = data.whattrbutecomp;
	    				var attrComps = data.attrcomps;
	    				addAttributeCompare(attrComps);
	    			}
	    		}    	
	        );
}
...
function addAttributeCompare(attrComps){
	if(null != attrComps){
		jQuery(attrComps).each(function(n){
			var obj = $("[name='" + this.keyCode + "']");
			if(obj != null && typeof obj != "undefined"){
				if(obj.attr('type') == 'text' || obj.attr('type') == 'TEXT'){
					obj.val(this.value);
				}else if(obj.attr('type') == 'checkbox' || obj.attr('type') == 'CHECKBOX'){
					checkBoxByAttr(obj, this.value);
				}else if(obj.attr('type') == 'radio' || obj.attr('type') == 'RADIO'){
					radioByAttr(obj, this.value);
				}else{
					selectOptionByAttr(obj, this.value);
				}
			}
		})
	}
...
function selectOptionByAttr(obj,value) {
	if(obj.find('option')){
		...
		if(null != count && count > 0){
			...
			if(!hasVal){
				...
				obj.get(0).options[count].selected = true;
			}
		}
	}
}  

function checkBoxByAttr(obj,value) {
	obj.each(
        function() {
        	if($(this).val() == value){
...
function radioByAttr(obj,value) {
	obj.each(
        function() {
        	if(this.value == value){
        		this.checked = true;
...
function deleteRows(rowId, weight){
	var listWeight = $('input[name=listWeight]').val();
	var weightLeft = accSubtr(listWeight, weight);
	$('input[name=listWeight]').val(weightLeft);
