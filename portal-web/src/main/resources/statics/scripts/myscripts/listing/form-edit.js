...
function validateForm() {
	...
	if (listingDateeStr != "") {
		 ...
		 * hh:mm")){ alert("挂牌开始时间不能小于当前时间！");
		 * $('input[name=listingDatesStr]').focus(); return; }
		 */
...
// 选择仓单后
function choiceBack(id, no, varietyCode, varietyName, material, listWeight,
		listNum) {
...
function unDirectWH() {
	...
	if (isDirectWhc == "0") { // 非指定交收库
		$("input[name=warehouseCmpId]").val("-1");
		$('input[name=warehouseCmpAddress]').val(
				$('input[name=settlementAddrS]').val());
		$('input[name=settlementAddr]').val(
				$('input[name=settlementAddrS]').val());
		$('input[name=warehouseCmpName]').val(
				$('input[name=warehouseCmpNameS]').val());
	}
}
...
// 创建表单
function createTableByVariety(receiptId, varietyCode, varietyName, listWeight,
		listNum, fn) {
	var id = $("input[name=id]").val();
	jQuery
			.get(
					appServer + '/listing/load_detail_' + id + '_edit_'
							+ receiptId + "_" + varietyCode + '.htm?'
							+ Math.round((Math.random()) * 100000000),
					{
						'listWeightS' : listWeight,
						'listNumS' : listNum
					},
					function(data) {
						var htmlCode = '';
						...
						if (receiptId != null) {
							$("input[type=radio][value=C]").attr("checked",
									'checked');
						} else {
							if (project.listingType == "D") {
								$("input[type=radio][value=D]").attr("checked",
										'checked');
							} else if (project.listingType == "G") {
								$("input[type=radio][value=G]").attr("checked",
										'checked');
						...
						$("input[name=warehouseCmp]").val(
								project.warehouseCmpId);

						formHtml = '<fieldset><legend>常规物品属性</legend><table class="c3">';
						jQuery(WP).each(function(i) {
							creatForm(this, i, WP.length, project)
						});
						...
						jQuery(GG).each(function(i) {
							creatForm(this, i, GG.length, project)
						});
						/*
						 * if($('input[name=listingType]:checked').val() =="W" ||
						 * $('input[name=listingType]:checked').val() =="F"){
						 * //库存挂牌,只显示“规格”属性，不显示长、宽、厚等 // formHtml += "<tr><th width=\"14%\">规格:</th><td ><input
						 * type='text' name='specification' value='"; // if(null !=
						 * project.specification && '' !=
						 * project.specification){ // formHtml +=
						 * project.specification; // } // formHtml += "'
						 * class=\"inpt \" style=\"width:126px;\" /></td></tr>"; }
						 */
						formHtml = formHtml + "</table></fieldset>";
						htmlCode = htmlCode + formHtml;
						...
						formHtml = formHtml
								+ "<tr><th>是否定向：</th><td>"
								+ "<input name=\"isDirect\" type=\"radio\" value=\"1\" onclick=\"isDirectClick();\" class=\"isDirect:true \" "
								+ isDirect1
								+ "/> 定向"
								+ "<input name=\"isDirect\" type=\"radio\" value=\"0\" onclick=\"isDirectClick();\" class=\"isDirect:true ml10\"  "
								+ isDirect0
								+ "/> 不定向 "
								+ "<div id=\"selectBuyerDiv\" style='"
								+ buyerDivDisplay
								+ "'>"
						...
						formHtml = formHtml
								...
								+ "<td><input type=\"text\" class=\"required  inpt\" onClick=\"WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'%y-%M-%d'})\" id=\"listingDatesStr\" name=\"listingDatesStr\"  style=\"width:126px;\"  value=\""
								+ Hundsun.formatDate(project.listingDates,
										"yyyy-MM-dd")
								+ "\" readOnly/><span class=\"red\"></span></td>"
								+ "<th><span style=\"color:red\">*</span>挂牌结束时间：</th>"
								+ "<td><input type=\"text\" class=\"required  inpt\" onClick=\"WdatePicker({dateFmt:'yyyy-MM-dd',minDate:$('#listingDatesStr').val()})\" id=\"listingDateeStr\" name=\"listingDateeStr\"   style=\"width:126px;\"  value=\""
								+ Hundsun.formatDate(project.listingDatee,
										"yyyy-MM-dd")
								+ "\" readOnly/><span class=\"red\"></span></td>"
								+ "</tr></table></fieldset>";
						...
						jQuery(WHCk).each(
								function(n) {
									...
									var tmpAddr = WHCk[n].provice
											+ WHCk[n].city + WHCk[n].area
											+ WHCk[n].address;
									tmpAddr = tmpAddr.replace(/null/g, "");
									if (project.warehouseCmpId == WHCk[n].id) {
										ops = ops + "<option selected value=\""
												+ WHCk[n].id + "|" + tmpAddr
												+ "\">" + WHCk[n].name
												+ "</option>";
						...
						jQuery(JS).each(function(i) {
							creatForm(this, i, JS.length, project)
						});
						if ($("#goodsType").val() == 1) {
							...
							formHtml = formHtml
									+ "<tr><th>是否允许自提：</th><td colspan='3'>"
									+ "<input id=\"selfDeliver1\" name=\"selfDeliver\" type=\"radio\" value=\"1\"  "
									+ selfDeliver1
									+ "/> 是   "
									+ "<input id=\"selfDeliver0\" name=\"selfDeliver\" type=\"radio\" value=\"0\"  "
									+ selfDeliver0 + "/> 否</td></tr>";
						}
						formHtml = formHtml
								+ "<tr><th>交收仓库：</th><td>"
								...
								+ "<td><input type=\"text\" class=\"required  inpt saveName\"  id=\"warehouseCmpNameS\" name=\"warehouseCmpNameS\"   style=\"width:126px;\" value=\""
								+ project.warehouseCmpName
								+ "\" readOnly /><span class=\"red\"></span>"
								+
								// "<a href='#0' class='button-2' id='btn_open'
								// onclick='selectCoop1();'>选择仓库</a></td>"+
								...
								+ "<td><input type=\"text\" class=\"required  inpt saveAdd \"  id=\"settlementAddrS\" name=\"settlementAddrS\"   style=\"width:126px;\"  value=\""
								+ project.settlementAddr
								+ "\" readOnly /><span class=\"red\"></span></td></tr>";
						...
						formHtml = '<fieldset><legend>其他属性</legend><table class="c3">'
								...
								+ "<td><textarea name=\"remark\" id=\"remark\" cols=\"80\" rows=\"3\" maxlength=\"340\" title=\"请不要超过340的长度\">"
								+ remark
								+ "</textarea><span class=\"red\"></span></td></tr>";
						formHtml = formHtml + "</table></fieldset>";
						htmlCode = htmlCode + formHtml;
						...
						if ('2' == varietyType) {
							...
							formHtml += '<tr style="border:1px solid #d1d7e0">'
									...
									+ '<td class="tc">重量</td>'
									+ '<td class="tc">状态</td>'
									+ '<td class="tc">入库时间</td>' + '</tr>';
							if (null != WHIItems && "" != WHIItems) {
								jQuery(WHIItems)
										.each(
												function(n) {
													formHtml += '<tr style="border:1px solid #d1d7e0" id="tr_'
															+ this.id
															+ '" >'
													 ...
													 * alert(this.batchNo);
													 * alert(this.weight);
													 * alert(this.statusDesc);
													 * alert(this.gmtCreate);
													 */
												})
							}
						...
						document.getElementById("varietyName_text").innerHTML = $(
								"input[name=varietyName]").val();
						...
						if ($("#isFullPayment").val() == "1"
								&& $("#isCreditTrade").val() == "1") {
							if (listingType == "D" || listingType == "G"
									|| listingType == "W" || listingType == "F") {
						...
						if (fn) {
							...
							if (warehouseCmpId == "-1" || warehouseCmpId == "0") {
								$("input[name=isDirectWhc]:eq(0)").attr(
										"checked", 'checked');
								isDirectWH();
							} else {
								document.getElementById("warehouseCmpTd").style.display = "";
								...
								var deliveryType = getDeliveryTypeCKValue();// $("#deliveryType").val();
								if (deliveryType.indexOf("1") >= 0) {
									$("input[name=isDirectWhc]:eq(1)").attr(
											"checked", 'checked');
									document.getElementById("isDirectWhc1").disabled = true;
									document.getElementById("isDirectWhc0").disabled = true;
								} else {
							...
							if (listingType == "C"
									&& ((receiptId != null && receiptId != "") || (project.receiptId != null && project.receiptId != "-1"))) {
								// 仓单挂牌时，材质不能选择只显示
						...
						if ($("#manufacuturer")) {// 选择生产商
							...
							// {$("#manufacuturer").after(" <a href='#0'
							// class='button-2 producer' id='btn_open'
							// onclick='selectCoop();'>选择</a>");}
							{
								$("#manufacuturer")
										.after(
...
function creatForm(attr, i, length, project) {
	...
	// 判断是否必填
	if (isRequired == "1") {
		appendHtml = appendHtml + " <th width=\"15%\">"
				+ "\<span style=\"color:red\">*</span>" + attr.keyTitle
				+ "：</th>";
	} else {
		appendHtml = appendHtml + " <th width=\"15%\">" + attr.keyTitle
				+ "：</th>";
	}
	...
	if (inputType == "TEXT") {
		...
		if (attr.keyCode == "varietyCode") {
			appendHtml = appendHtml
					...
					+ "  class=\"inpt  "
					+ clas
					+ "\" style=\"width:126px;\" /><span id=\"varietyName_text\"></span> "
					+ unitType + " <span class=\"red\"></span>";
		} else {
			appendHtml = appendHtml + "<input type=\"text\"  id=\""
					+ attr.keyCode + "\" name=\"" + attr.keyCode
					+ "\" value=\"" + value + "\"   " + readOnly + " "
					+ onblurFunc + " class=\"inpt  " + clas
					+ "\" style=\"width:126px;\" />" + unitType
					+ " <span class=\"red\"></span>";
		}
	} else if (inputType == "TEXTAREA") {// 输入框为文本域
		appendHtml = appendHtml + "<textarea rows=\"2\" cols=\"60\"  id=\""
				+ attr.keyCode + "\" name=\"" + attr.keyCode
		...
		if (project && project.listingType == "C"
			...
			appendHtml = appendHtml
					+ "<input type=\"text\" maxlength=\"100\" id=\""
					+ attr.keyCode + "\" name=\"" + attr.keyCode
					+ "\"   readOnly  value=\"" + value + "\" class=\"inpt "
					+ clas
					+ "\" style=\"width:126px;\" /><span class=\"red\"></span>";
		} else {
			appendHtml = appendHtml
...
function chooseVType(data) {
	...
	if (varietyCode != data.code) {
		jQuery
				.get(
						appServer + '/listingRemoting/load_variety_'
								+ data.code + '.htm',
						function(data) {
							var WP = data.F_WP;
							if (WP == "") {
								...
								createTableByVariety(null, data.code,
