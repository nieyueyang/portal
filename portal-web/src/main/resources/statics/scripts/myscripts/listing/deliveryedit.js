	...
	$("#submitBtn")
			.click(
					function() {
						...
						var formId = "";
						var deliveryType = "";
						if ($("#preInputForm input[name='deliveryType']").val() == "1") {
...
function getDeliveryTable(index) {
	...
	if (varietyRecordList[index].length > 0) {
		for ( var i = 0; i < varietyRecordList[index].length; i++) {
			...
			htmlStr += "tr" + i + "'>";
			htmlStr += "<td>";
			var name = varietyRecordList[index][i].name;
			if (isNotBlank(name)) {
				htmlStr += name;
			}
			htmlStr += "</td>";
			htmlStr += "<td>";
			var carNo = varietyRecordList[index][i].carNo;
			if (isNotBlank(carNo)) {
				htmlStr += carNo;
			}
			htmlStr += "</td>";
			htmlStr += "<td>";
			var comments = varietyRecordList[index][i].comments;
