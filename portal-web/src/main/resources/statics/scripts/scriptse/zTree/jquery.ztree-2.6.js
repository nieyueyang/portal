...
(function($) {
	...
	function appendTreeNodes(setting, level, treeNodes, parentNode) {
		...
		for (var i = 0, l = treeNodes.length; i < l; i++) {
			...
			//added by tanhl 2012-11-21 start，只有叶子节点才显示checkbox, 通过ztree里面的属性isParent判断，false即为叶子节点
			if (setting.checkLeafOnly) {
			...
			if(setting.checkFromLevel != -1) {
				if(node.level >= setting.checkFromLevel) {
					makeChkFlag(setting, node);
					if (setting.checkStyle == Check_Style_Radio && setting.checkRadioType == Radio_Type_All && node[setting.checkedCol] ) {
						setting.checkRadioCheckedList = setting.checkRadioCheckedList.concat([node]);
					}
	...
	function eventProxy(event) {
		...
		if (tools.eqs(event.type, "mouseover")) {
			...
			} else if ((setting.checkLeafOnly || setting.checkFromLevel != -1)  && tools.eqs(target.tagName, "button") && target.getAttribute("treeNode"+IDMark_Check) !== null) {
				//added by tanhl ，使checkbox勾选有效
				tId = target.parentNode.id;
				childEventType = "checkNode";
			} else {
				tmp = tools.getMDom(setting, target, [{tagName:"a", attrName:"treeNode"+IDMark_A}]);
	...
	function repairAllChk(setting, checked) {
		...
		//added by tanhl 修正叶子节点勾选
		if (setting.checkLeafOnly || setting.checkFromLevel != -1) {
			for (var son = 0; son < setting.root[setting.nodesCol].length; son++) {
				var treeNode = setting.root[setting.nodesCol][son];
				//遍历全选/反选子节点
				setLeafNodeCheckBox(setting, treeNode, checked);
			}
		}
	}
	 ...
	 * @author tanhl
	 */
	function setLeafNodeCheckBox(setting, treeNode, value) {
		var checkObj = $("#" + treeNode.tId + IDMark_Check);
		//如果是叶子节点
		if(!treeNode.isParent) {
			treeNode[setting.checkedCol] = value;
		...
		//递归遍历勾选
		for(var i=0; i<treeNode[setting.nodesCol].length; i++) {
			setLeafNodeCheckBox(setting, treeNode[setting.nodesCol][i], value);
		}
	}
