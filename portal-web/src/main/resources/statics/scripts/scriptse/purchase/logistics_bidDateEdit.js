		...
		buttons : {
			"确认" : function() {
				...
				$("#secondBidEndDateMsg").html("");
				 ...
				 var basePriceArr = $(":input[name='basePrice']");
				 ...
				 for(var i=0;i<secondPriceArr.length;i++) {
					 ...
					 if( parseFloat(secondPriceArr[i].value*100)>parseFloat(basePriceArr[i].value) )
	                 {  
						 secondPriceMsgArr[i].innerHTML = "不能大于拦标价";
						 //flag = false;
						 return false;
					 } else {
						 secondPriceMsgArr[i].innerHTML = "";
...
function editBidEndDate(tenderId, currentDate, beginDate,status,isAutoBargain) {
	//自动成交，非新建状态，当前时间大于投标结束时间，则不允许修改时间
	if(status!='0' && isAutoBargain=='0'){
		var time = new Date().getTime();
		var ctime=new Date(currentDate).getTime();
		if(ctime<time){
			 alert("已自动成交,不允许修改投标结束时间");
			 return;
		 }
	}
...
function secondBidBeginDate(tenderId, tenderNo, currentDate, endDate, num) {
	$.ajax({
		url: appServer+"/logistics/buyer/getPlans.htm?tenderNo=" + tenderNo,
		...
		success: function (map) {
			...
			for ( var i=0; i<list.length; i++) {
				...
				str += "<tr>" +
						"<td>" +
							...
							"<input type='hidden' value='"+list[i].basePrice+"' name='basePrice' />" +
							...
							"<div style='width:270px;float:left;'>目前最低价：<input name='secondLowPrice' class='inpt' type='text' value='"+secondPrice+"'/>"+"元" +
			...
			$("#companyList").append("<tr>" +
					"<td style='font-weight:bold;'>承运方列表：</td>" +
					"</tr>" +
					cStr);
