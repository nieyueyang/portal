...
function callbackforadd(result) {
	...
	if (result.data != "true") {
		...
		Hundsun.PopUtil.alert({
			...
			autohide : false,
			callback : function() {
				if(result.bankReturnUrl != ""){
					Hundsun.UrlUtil.redirect(result.bankReturnUrl);
				}else{
									...
									if (data.data != "true") {
										Hundsun.PopUtil
												.alert({
													...
													callback : function() {
														Hundsun.UrlUtil
																.redirect("/contract/buyer/goRekonApplyContract.htm?id="+contractId);
													}
												});
									} else {
										Hundsun.PopUtil
